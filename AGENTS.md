...
3. For engine/connector changes: exercise the flow by uploading a report or collecting from a source, then «تشغيل التحليل» (run analysis) with a live LLM backend, or use «اختبار المحرّك» / «اختبار البريد» in Settings to test the engine and email independently.
